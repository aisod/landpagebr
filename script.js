...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Auto-scroll fleet showcase in an infinite loop
    const fleetGrid = document.querySelector('.fleet-grid');
    if (fleetGrid) {
        // Duplicate cards to enable seamless looping
        const fleetCards = Array.from(fleetGrid.children);
        const fleetCloneFragment = document.createDocumentFragment();
        fleetCards.forEach(card => fleetCloneFragment.appendChild(card.cloneNode(true)));
        fleetGrid.appendChild(fleetCloneFragment);

        // Disable snap during auto-scroll for smoothness
        fleetGrid.style.scrollSnapType = 'none';

        let fleetIsPaused = false;
        let fleetScrollSpeed = 0.8; // adjust speed

        // Pause on hover/touch
        fleetGrid.addEventListener('mouseenter', () => (fleetIsPaused = true));
        fleetGrid.addEventListener('mouseleave', () => (fleetIsPaused = false));
        fleetGrid.addEventListener('touchstart', () => (fleetIsPaused = true), { passive: true });
        fleetGrid.addEventListener('touchend', () => (fleetIsPaused = false));

        // Pause auto-scroll when the fleet section is not visible
        const fleetVisibilityObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                fleetIsPaused = !entry.isIntersecting;
            });
        }, { threshold: 0.1 });
        fleetVisibilityObserver.observe(fleetGrid);

        function fleetLoopScroll() {
            if (!fleetIsPaused) {
                fleetGrid.scrollLeft += fleetScrollSpeed;
                const halfWidth = fleetGrid.scrollWidth / 2;
                if (fleetGrid.scrollLeft >= halfWidth) {
                    fleetGrid.scrollLeft = 0;
                }
            }
            requestAnimationFrame(fleetLoopScroll);
        }
        requestAnimationFrame(fleetLoopScroll);
    }
});
